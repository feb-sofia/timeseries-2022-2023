...
ar1 <- 100 + arima.sim(model = list(order = c(1, 0, 0), ar = 0.7), n = 200)
plot(ar1)
```
...
```{r}
set.seed(5454) ## For reproducibility of the simulation results
ar2 <- 10 + 0.1*(1:200) + arima.sim(model = list(order = c(1, 0, 0), ar = 0.7), n = 200)
plot(ar2)
```
...
```{r}
ar3 <- arima.sim(n = 200, model = list(ar = c(0.6, 0.4, -0.7)))

summary(ur.df(ar3, type = "none"))
```
