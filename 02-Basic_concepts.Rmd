...
$$
where $T$ is the index of the last observation. In the bitcoins example $T = ?$. $y_1 = ?, y_2 = ?, \ldots, y_{?} = ?$.
...
The sign of this sum in the covariance definition depends on the arrangement of the observations relative to the
series average.
...
## Simulation: a purely random series
...
ts_random <- xts(
  xrandom,
  order.by = seq.Date(from = as.Date("2017-06-01"), by = "day", length.out = n)
)
```
